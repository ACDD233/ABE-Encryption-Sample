...
#### **创建子管理员 (Create Sub-Admin)**
创建具有 `SUB_ADMIN` 角色的新用户。
- **端点**: `POST /admin/subadmin`
- **请求体 (JSON)**:
  ```json
  {
    "username": "hr_admin",
  ...
  ```
- **权限逻辑**: 
  - **子管理员 (Sub-Admin)** 可以管理其他用户，但权限受到限制。
  - 他们只能向用户分配 **他们自己持有** 的属性。
  - 他们 **不能** 在目录中创建新属性，也不能删除用户。

#### **列出所有用户 (List Users)**
- **端点**: `GET /admin/users`
- **响应**: `User` 对象列表。
- **子管理员行为**: 当由子管理员调用时，列表会经过过滤，仅显示普通用户（不显示其他管理员或子管理员）。
